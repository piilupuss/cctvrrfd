...
- Multi-threaded CCTV Stream Capture
...
RESULTS_FILE = "data/flood_detection_results.csv" # CSV to log detection results
CCTV_CSV = "data/cctv_locations.csv" # CSV with CCTV details (ID, Name, Lat, Lon, URL)
CAPTURE_INTERVAL = 20  # Capture interval in seconds per CCTV
...
def get_risk_roads_by_elevation(_roads_gdf, flood_point_df, dem_data, dem_transform, radius_meters=FLOOD_RISK_RADIUS_M):
    ...
    for idx, road in buffered_roads_projected.iterrows():
        ...
        for lon, lat in coords:
            ...
            if road_elev is not None and road_elev < flood_point_df['flood_elevation']:
                is_risky = True
                break
...
def main():
    st.set_page_config(layout="wide", page_title="Bandung Flood Monitoring")
    st.title("Bandung Flood Monitoring")
    # st.markdown("""
    # Real-time flood spread estimation using CCTV streams, true IDW interpolation, and DEM data.
    # """)
    st.markdown("""
    Real-time flood status from CCTV streams in Bandung City.
    """)
    ...
    while True:
        ...
        while not cctv_status_queue.empty():
            ...
            global_cctv_statuses[new_status['cctv_id']] = new_status # Update or add status
